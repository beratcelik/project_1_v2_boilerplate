...
class Blockchain {
        ...
        return new Promise(async (resolve, reject) => {
            if(currentTime - time < 5*600000){
                if(bitcoinMessage.verify(message, address, signature)){
                    ...
                    reject("star not verified");
                }

            }else{
                resolve("It is too late: "+ (currentTime - time)+", time of block: "+time+", current time: "+ currentTime);
            }
        });
    }
    ...
    getBlockByHash(hash) {
        let self = this;
        return new Promise((resolve, reject) => {
           let blockHasHash = self.chain.filter(p => p.hash === hash)[0];
           if(blockHasHash){
               resolve(blockHasHash);
           }else{}
                resolve(null);
        });
    }
